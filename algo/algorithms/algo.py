...
class MazeSolver:
    ...
    def get_optimal_path(self) -> tuple[list[CellState], float]:
        ...
        # optimal_path = self._combine_straight_segments(optimal_path)
    ...
    def _combine_straight_segments(self, path: list[CellState]) -> list[CellState]:
        """
        Combine consecutive straight-line segments in the path into a single segment.
        Only keep the start of a straight segment and the last cell before a turn or stop.
        """
        ...
        for i in range(1, len(path) - 1):
            ...
            curr = path[i]
            nxt = path[i + 1]
            # Check if prev->curr->nxt are all in a straight line (same direction)
            dx1 = curr.x - prev.x
            dy1 = curr.y - prev.y
            dx2 = nxt.x - curr.x
            dy2 = nxt.y - curr.y
            dir1 = curr.direction
            dir2 = nxt.direction
            # If direction changes or movement direction changes, keep curr
            if (dx1, dy1) != (dx2, dy2) or dir1 != dir2:
                combined.append(curr)
            # If a screenshot is taken at curr, keep it
            elif getattr(curr, 'screenshot_id', None):
                combined.append(curr)
        combined.append(path[-1])
        return combined
