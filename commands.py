...
class CommandGenerator:
    ...
    def _generate_command(self, motion: Motion, num_motions: int = 1) -> list[str]:
        """Generate simplified commands: one letter + one argument."""
        if num_motions > 1:
            dist = num_motions * self.UNIT_DIST
        else:
            dist = self.UNIT_DIST
        ...
        if motion == Motion.FORWARD:
            return [f"T{int(dist)}"]  # forward with distance
        elif motion == Motion.REVERSE:
            return [f"t{int(dist)}"]  # backward with distance
        elif motion == Motion.FORWARD_LEFT_TURN:
            return [f"L90"]  # left turn with angle (e.g., 90 degrees)
        elif motion == Motion.FORWARD_RIGHT_TURN:
            return [f"R90"]  # right turn with angle
        elif motion == Motion.REVERSE_LEFT_TURN:
            return [f"l90"]  # backward left turn with angle
        elif motion == Motion.REVERSE_RIGHT_TURN:
            return [f"r90"]  # backward right turn with angle
        else:
